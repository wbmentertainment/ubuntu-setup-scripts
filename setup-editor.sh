...
ExecStartPre=/usr/bin/mount -t cifs -o username=admin1,password=Came2020,vers=3.0,rw,dir_mode=0777,file_mode=0777 //192.168.1.111/media-editor ${PROJECT_DIR}/NAS
ExecStart=/bin/bash ${PROJECT_DIR}/startup.sh
StandardOutput=append:/var/log/media-editor.log
StandardError=append:/var/log/media-editor.log
...
[Unit]
Description=Backup Mongo (editor) to NAS
After=network-online.target docker.service
...
Type=oneshot
User=wbm
WorkingDirectory=/home/wbm/projects/media-editor
ExecStart=/bin/bash /home/wbm/projects/media-editor/backup-mongo.sh
EOF
...
[Unit]
Description=Run Backup Mongo (editor) daily at 02:00

[Timer]
OnCalendar=*-*-* 02:00:00
Persistent=true
Unit=media-editor-backup.service
